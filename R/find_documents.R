...
#' @return A vector of filenames for journal documents within the range.
get_journals_in_range <- function(start_date, end_date, root_dir) {
  all_docs <- list.files(root_dir, full.names = TRUE)
  journal_pattern <- "notebook\\d{8}\\.Rmd"
  
  journal_docs <- sapply(all_docs, function(doc) {
  ...
  journal_docs <- journal_docs[!sapply(journal_docs, is.null)]
...
#' @return A vector of filenames for documents within the range.
#' @export
get_documents_in_range <- function(start, end = lubridate::now(), 
  ...
  docs_in_range <- sapply(all_docs, function(doc) {
  ...
  docs_in_range <- docs_in_range[!sapply(docs_in_range, is.null)]
